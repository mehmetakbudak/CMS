      ...
      <div class="container">
        <div class="row">
          <div class="col-lg-8 entries">
            <DataView :value="blogCategory.blogs" layout="grid" :paginator="true" :rows="5">
                ...
                <ul>
                  ...
                  >
                    <router-link :to="`/blog/${item.url}`"
                      >{{ item.name }}
                      <span>({{ item.blogCount }})</span></router-link
                    >
                  </li>
                </ul>
              </div>
...
export default {
  ...
  methods: {
    getAll() {
      var url = this.$route.params.categoryUrl;
      GlobalService.Get(`${Endpoints.Blog}/GetBlogsByCategoryUrl/${url}`)
        .then((res) => {
    ...
    mostRead() {
      var url = this.$route.params.categoryUrl;
      GlobalService.Get(`${Endpoints.Blog}/MostReadByBlogCategory/${url}`).then(
        (res) => {
